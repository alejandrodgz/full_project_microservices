...
func main() {
	...
	// Inicializar citizen validator
	var citizenValidator external.CitizenValidator
	if cfg.ExternalAPI.EnableValidation {
		logger.Info("Citizen validation enabled",
			zap.String("api_url", cfg.ExternalAPI.CitizenValidationURL))
		citizenValidator = external.NewGovcarpetaValidator(
			cfg.ExternalAPI.CitizenValidationURL,
			cfg.ExternalAPI.CitizenValidationAPIKey,
			logger,
		)
	} else {
		logger.Info("Citizen validation disabled")
	}
	...
	authService := services.NewAuthService(
		...
		jwtService,
		citizenValidator,
		cfg.ExternalAPI.EnableValidation,
		logger,
	)
