...
import (
	...
	"github.com/kristianrpo/auth-microservice/internal/infrastructure/external"
...
type AuthService struct {
	...
	citizenValidator external.CitizenValidator
...
func (s *AuthService) Register(ctx context.Context, email, password, name string, idCitizen int) (*domain.UserPublic, error) {
	...
	// Validate citizen with external API if enabled
	if s.enableValidation && s.citizenValidator != nil {
		s.logger.Info("validating citizen with external API", zap.Int("id_citizen", idCitizen))
		
		validationResp, err := s.citizenValidator.ValidateCitizen(ctx, idCitizen)
		if err != nil {
			s.logger.Error("failed to validate citizen with external API", 
				zap.Error(err), 
				zap.Int("id_citizen", idCitizen))
			return nil, domainerrors.ErrCitizenValidationError
		}

		// If citizen already exists in the external system, reject registration
		if validationResp.Exists {
			s.logger.Warn("citizen already registered in external system, cannot create duplicate", 
				zap.Int("id_citizen", idCitizen),
				zap.String("message", validationResp.Message))
			return nil, domainerrors.ErrUserAlreadyExists
		}

		s.logger.Info("citizen validation successful - can proceed with registration", 
			zap.Int("id_citizen", idCitizen),
			zap.Bool("exists", validationResp.Exists))
	} else {
		s.logger.Debug("citizen validation disabled or validator not available")
	}
