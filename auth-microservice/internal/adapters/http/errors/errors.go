...
var (
	...
	ErrCitizenNotFound       = NewHTTPError(nethttp.StatusBadRequest, "Citizen ID not found in the government system", "CITIZEN_NOT_FOUND")
	ErrCitizenValidation     = NewHTTPError(nethttp.StatusServiceUnavailable, "Failed to validate citizen with external system", "CITIZEN_VALIDATION_ERROR")
)
...
func MapDomainError(err error) *HTTPError {
	...
	case errors.Is(err, domainerrors.ErrCitizenNotFound):
		return ErrCitizenNotFound
	case errors.Is(err, domainerrors.ErrCitizenValidationError):
		return ErrCitizenValidation
	default:
		// Error genérico
