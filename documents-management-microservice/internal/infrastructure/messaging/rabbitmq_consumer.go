...
func (r *RabbitMQConsumer) setupConsumer(queueName string) error {
	...
	// Declare the exchange if configured
	if cfg.Exchange != "" {
	...
	if cfg.Exchange != "" {
		...
		if err != nil {
			return fmt.Errorf("failed to bind queue %s to exchange %s: %w", queueName, cfg.Exchange, err)
		}
		log.Printf("Queue %s bound to exchange %s with routing key %s", queueName, cfg.Exchange, queueName)
	}
