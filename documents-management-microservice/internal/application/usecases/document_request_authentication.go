...
func (s *documentRequestAuthenticationService) RequestAuthentication(
	...
	// Publish to exchange with routing key (for topic-based routing to Affiliation microservice)
	// Type assert to access PublishToExchange method
	type exchangePublisher interface {
		PublishToExchange(ctx context.Context, exchange string, routingKey string, message []byte) error
	}
	
	if ep, ok := s.publisher.(exchangePublisher); ok && s.exchange != "" {
		// Use exchange-based publishing
		if err := ep.PublishToExchange(ctx, s.exchange, s.routingKey, eventJSON); err != nil {
			return fmt.Errorf("failed to publish authentication request event to exchange: %w", err)
		}
	} else {
		// Fallback to direct queue publishing
		if err := s.publisher.Publish(ctx, s.routingKey, eventJSON); err != nil {
