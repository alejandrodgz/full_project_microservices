...
from apps.documents.services import DocumentAuthenticationService
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        def process_message(message: dict):
            ...
            try:
                # Support both camelCase (from Go) and PascalCase formats
                id_citizen = message.get('idCitizen') or message.get('IDCitizen')
                url_document = message.get('urlDocument') or message.get('UrlDocument')
                document_title = message.get('documentTitle') or message.get('DocumentTitle')
                message_id = message.get('messageId') or message.get('MessageID', '')
                document_id = message.get('documentId') or message.get('DocumentID', '')
